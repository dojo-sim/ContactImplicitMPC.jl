...
# get ref. trajectory
ref_traj = ContactImplicitMPC.get_trajectory("particle", "sinusoidal2",
...
# # simulator
# sim = ContactImplicitMPC.simulator(model, q0, q1, 1.0 * h, H,
#     p = ContactImplicitMPC.open_loop_policy([SVector{model.dim.u}(ut) for ut in ref_traj.u], N_sample = 1),
#     ip_opts = ContactImplicitMPC.InteriorPointOptions(r_tol = 1.0e-8, κ_init = 1.0e-5, κ_tol = 1.0e-6),
...
obj = ContactImplicitMPC.TrackingObjective(H, model.dim,
...
# simulator
sim = ContactImplicitMPC.simulator(model, q0_sim, q1_sim, h_sim, H_sim,
    p = p,
