...
function obj1(x, u, w)
    ...
    J += 0.5 * transpose(u[model.nu + 8 .+ (1:40)]) * Diagonal(1.0 * ones(40)) * u[model.nu + 8 .+ (1:40)]
...
contact_constraints_inequality_t(model, env, h, rand(nx + nθ + nx), rand(nu), zeros(0))
contact_constraints_inequality_T(model, env, h, rand(nx + nθ + nx), rand(nu), zeros(0))
...
con1 = DTO.Constraint(constraints_1, nx, nu, idx_ineq=collect(32 .+ (1:56)))
cont = DTO.Constraint(constraints_t, nx + nθ + nx, nu, idx_ineq=collect(32 .+ (1:64)))
conT = DTO.Constraint(constraints_T, nx + nθ + nx, nu, idx_ineq=collect(0 .+ (1:16)))
...
centroidal_quadruped_wall.mass_foot
