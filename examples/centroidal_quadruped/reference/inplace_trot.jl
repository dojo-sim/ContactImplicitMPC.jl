...
for t = 1:T
    ...
    elseif t == T
        ...
        push!(cons, DTO.Constraint(constraints_T, nx + nθ + nx + model.nu, nu, indices_inequality=collect(0 .+ (1:8))))
    else
        function constraints_t(x, u, w)
            [
            # equality (16)
        ...
        push!(cons, DTO.Constraint(constraints_t, nx + nθ + nx + model.nu, nu, indices_inequality=collect(16 .+ (1:32))) )
    end
end
...
plot(timesteps, hcat(γ_opt...)', labels="")
plot(timesteps, hcat(b_opt...)', labels="")
plot(timesteps, hcat(ψ_opt...)', labels="")
plot(timesteps, hcat(η_opt...)', labels="")
...
if mode == :right
    ...
    using JLD2
    @save joinpath(@__DIR__, "inplace_trot_right.jld2") qqr ur γr br ψr ηr
    @load joinpath(@__DIR__, "inplace_trot_right.jld2") qqr ur γr br ψr ηr
end
