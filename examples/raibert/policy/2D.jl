...
mutable struct Raibert <: ContactImplicitMPC.Policy
    kr_c::Real
    kr_p::Real
    kr_v_stance::Real
    kr_v_flight::Real
...
function raibert_policy(model::ContactImplicitMPC.Hopper2D;
...
function ContactImplicitMPC.policy(p::Raibert, x, traj, t)
    # Initialization
    h = traj.h
    p.q0 = copy(traj.q[t])
    p.q1 = copy(x)
