...
@testset "Newton: Residual and Jacobian" begin
	...
	# Test Residual
	r0 = ContactImplicitMPC.NewtonResidual(model, env, H)
	...
	# Test Jacobian
	j0 = ContactImplicitMPC.NewtonJacobian(model, env, H)
...
@testset "Newton: jacobian!" begin
	...
	im_traj0 = ContactImplicitMPC.ImplicitTrajectory(ref_traj, s)
	...
	for t = 3:H
		@test all(core.jac.q0[t-2] .== im_traj0.δq0[t][1:nq, :])
	end

	for t = 2:H
		@test all(core.jac.q1[t-1] .== im_traj0.δq1[t][1:nq, :])
	end

	for t = 1:H
		@test all(core.jac.u1[t] .== im_traj0.δu1[t][1:nq, :])
	end
end
...
@testset "Newton: residual!" begin
	...
	@test (norm(core.res.u1[1] .- core.obj.u[1] * core.Δu[1]
		- im_traj0.δu1[1][1:nq, :]' * core.ν[1], Inf) < 1.0e-8)
end
