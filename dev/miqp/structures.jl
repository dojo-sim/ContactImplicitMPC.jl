...
mutable struct WallPendulum12{T} <: ContactModel
	n::Int # state dim
	m::Int # control dim
	mp::T # mass
	l::T # length
...
function dynamics_model(model::WallPendulum12{T}, dt::T; mode::Symbol = :none) where {T}
	mp = model.mp
	l = model.l
	g = model.g
	k = model.k
...
function get_mode(x::AbstractVector{T}, u::AbstractVector{T}, model::WallPendulum12{T}) where {T}
	d = model.d
	l = model.l
	if -d/l <= x[1] <= d/l # none
		return 1
...
function dynamics(x0::AbstractVector{T}, u0::AbstractVector{T},
		...
		model::WallPendulum12{T}) where {T}
...
function domain(model::WallPendulum12{T}; mode::Symbol = :none) where{T}
	m = model.m
	l = model.l
	g = model.g
	k = model.k
