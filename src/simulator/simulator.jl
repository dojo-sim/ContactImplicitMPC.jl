...
function step!(sim::Simulator, t)
    ...
    if status
        ...
        if sim.ip.opts.diff_sol
            ...
            sim.traj_deriv.dq2dq0[t] = view(ip.δz, 1:nq, 1:nq)
            sim.traj_deriv.dq2dq1[t] = view(ip.δz, 1:nq, nq .+ (1:nq))
            sim.traj_deriv.dq2du[t] = view(ip.δz, 1:nq, 2 * nq .+ (1:nu))
            sim.traj_deriv.dγdq0[t] = view(ip.δz, nq .+ (1:nc), 1:nq)
            sim.traj_deriv.dγdq1[t] = view(ip.δz, nq .+ (1:nc), nq .+ (1:nq))
            sim.traj_deriv.dγdu[t] = view(ip.δz, nq .+ (1:nc), 2 * nq .+ (1:nu))
            sim.traj_deriv.dbdq0[t] = view(ip.δz, nq + nc .+ (1:nb), 1:nq)
            sim.traj_deriv.dbdq1[t] = view(ip.δz, nq + nc .+ (1:nb), nq .+ (1:nq))
            sim.traj_deriv.dbdu[t] = view(ip.δz, nq + nc .+ (1:nb), 2 * nq .+ (1:nu))
        end
    end
    return status
end
