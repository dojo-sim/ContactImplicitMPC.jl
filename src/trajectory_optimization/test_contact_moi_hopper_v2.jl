...
d = gen_dynamics(s, h,
	...
	jac_opts = InteriorPointOptions{Float64}(κ_tol = 1.0e-1, κ_init = 0.1))
...
function obj_config(q, qref)
	Qt = Diagonal([1.0; 0.1; 0.5; 10.0])
	return transpose(q - qref) * Qt * (q - qref)
end

function obj_configT(q)
	QT = Diagonal([1.0; 1.0; 1.0; 1.0])
...
function moi_obj(z)
	...
	for t = 1:T
		qt = z[q_idx[t]]
		J += t == T ? obj_configT_func(qt) : obj_config_func(qt, q_ref)
	end
...
function ∇moi_obj!(g, z)
	for t = 1:T
		qt = z[q_idx[t]]
		g[q_idx[t]] .= t == T ? obj_configT_func(qt) : obj_config_q_func(qt, q_ref)
	end
