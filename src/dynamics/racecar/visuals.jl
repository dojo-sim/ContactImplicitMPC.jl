function plot_lines!(vis::Visualizer, model::RaceCar, q::AbstractVector;
		r_wheel=0.10, offset=0.02, size=10, name::Symbol=:racecar, col::Bool=true)
	p_shift = [0.0, 0.0, r_wheel]
...
function build_robot!(vis::Visualizer, model::RaceCar; name::Symbol=:RaceCar, r=0.10, r_sus=0.02, d=0.20, α=1.0)
	r = convert(Float32, r)
    ...
    setobject!(vis[name][:robot]["body"], HyperRectangle(-Vec(d, d, d)/2,
		Vec(d, d, d)), body_mat)
    setobject!(vis[name][:robot]["wheel"], Sphere(Point3f0(0), r), contact_mat)
    setobject!(vis[name][:robot]["suspension"], Cylinder(Point3f0(0),
		Point3f0(0, 0, 1.0), r_sus), body_mat)
	return nothing
end

function set_robot!(vis::Visualizer, model::RaceCar, q::AbstractVector;
		name::Symbol=:RaceCar, r=0.10, r_sus=0.02, d=0.20)
	r = convert(Float32, r)
	r_sus = convert(Float32, r_sus)
	d = convert(Float32, d)
	p_shift = [0.0; 0.0; r]
    ...
    settransform!(vis[name][:robot]["wheel"], Translation(p_wheel + p_shift))
	settransform!(vis[name][:robot]["suspension"],
		compose(
			cable_transform(p_wheel + 1e-10rand(3) + p_shift, p_body + p_shift),
			LinearMap(Diagonal([1,1,suspension_length])))
		)
	return nothing
end

function contact_point(model::RaceCar, q::AbstractVector)
	p_wheel = [kinematics(model, q)[1], 0.0, kinematics(model, q)[2]]
	pc = [p_wheel]
	return pc
end
