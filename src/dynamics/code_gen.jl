...
function generate_dynamics_expressions(model::ContactDynamicsModel)
	...
	# L_expr = build_function(transpose([L]), q, q̇)[1] # need to transpose to get a line vector
	# M_expr = build_function(M, q)[1]
	# B_expr = build_function(B, q)[1]
	# N_expr = build_function(N, q)[1]
	# P_expr = build_function(P, q)[1]
	# C_expr = build_function(C, q, q̇)[1]
