function plot_lines!(vis::Visualizer, model::PinnedBiped513, q::AbstractVector;
		r=0.035, offset=0.05, size=10, name::Symbol=:PinnedBiped513, col::Bool=true, α=1.0)
	r_contact = r*8/7
	...
	for qi in q
		k_torso = kinematics__3(model, qi, body = :torso, mode = :com)
		p_torso = [k_torso[1], -offset, k_torso[2]] + p_shift

		# k_toe_1 = kinematics__1(model, qi, body = :calf_1, mode = :ee)
		k_toe_1 = [0.0, 0.0]
		p_toe_1 = [k_toe_1[1], -offset, k_toe_1[2]] + p_shift

		k_toe_2 = kinematics__4(model, qi, body = :calf_2, mode = :ee)
...
function build_robot!(vis::Visualizer, model::PinnedBiped513; name::Symbol=:PinnedBiped513, r=0.035)
	r = convert(Float32, r)
	...
	point_mat = MeshPhongMaterial(color = RGBA(1.0, 0.0, 0.0, 1.0))
	...
	setobject!(vis[name][:robot]["com"], Sphere(Point3f0(0.0), r_contact), point_mat)

	return nothing
end

function set_robot!(vis::Visualizer, model::PinnedBiped513, q::AbstractVector;
		name::Symbol=:PinnedBiped513, r=0.035)

	r = convert(Float32, r)
	r_contact = convert(Float32, r*8/7)
	p_shift = [0.0; 0.0; r_contact]

	p = [0.0; 0.0; 0.0] + p_shift

	k_calf_1 = kinematics__1(model, q, body = :calf_1, mode = :ee)
	p_calf_1 = [k_calf_1[1], 0.0, k_calf_1[2]] + p_shift

	k_thigh_1 = kinematics__2(model, q, body = :thigh_1, mode = :ee)
	p_thigh_1 = [k_thigh_1[1], 0.0, k_thigh_1[2]] + p_shift

	k_torso = kinematics__3(model, q, body = :torso, mode = :ee)
	p_torso = [k_torso[1], 0.0, k_torso[2]] + p_shift

	k_thigh_2 = kinematics__3(model, q, body = :thigh_2, mode = :ee)
	p_thigh_2 = [k_thigh_2[1], 0.0, k_thigh_2[2]] + p_shift

	k_calf_2 = kinematics__4(model, q, body = :calf_2, mode = :ee)
	p_calf_2 = [k_calf_2[1], 0.0, k_calf_2[2]] + p_shift

	p_com = cast3d(com_func(model, q))

	settransform!(vis[name][:robot]["thigh1"], cable_transform(p_calf_1, p_thigh_1))
	settransform!(vis[name][:robot]["calf1"], cable_transform(p, p_calf_1))

	settransform!(vis[name][:robot]["thigh2"], cable_transform(p_thigh_1, p_thigh_2))
	settransform!(vis[name][:robot]["calf2"], cable_transform(p_thigh_2, p_calf_2))

	settransform!(vis[name][:robot]["torso"], cable_transform(p_torso,p_thigh_1))
	settransform!(vis[name][:robot]["toe1"], Translation(p))
	settransform!(vis[name][:robot]["toe2"], Translation(p_calf_2))
	settransform!(vis[name][:robot]["knee1"], Translation(p_calf_1))
	settransform!(vis[name][:robot]["knee2"], Translation(p_thigh_2))
	settransform!(vis[name][:robot]["hip"], Translation(p_thigh_1))
	settransform!(vis[name][:robot]["torso_top"], Translation(p_torso))

	settransform!(vis[name][:robot]["com"], Translation(p_com))

	return nothing
end

function contact_point(model::PinnedBiped513, q::AbstractVector)
	# k_toe_1 = kinematics__1(model, q, body = :calf_1, mode = :ee)
	k_toe_1 = [0.0, 0.0]
	p_toe_1 = [k_toe_1[1], 0.0, k_toe_1[2]]

	k_toe_2 = kinematics__4(model, q, body = :calf_2, mode = :ee)
