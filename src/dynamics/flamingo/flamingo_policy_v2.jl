"""
    Flamingo25 hopper policy for Hopper2D
"""

@with_kw mutable struct Flamingo25Options
    live_plotting::Bool=false # Use the live plotting tool to debug
end

mutable struct Flamingo25 <: Policy
	model::Flamingo
	phase::Symbol
	count::Int
	h_sim::Real
...
function flamingo_policy(model::Flamingo, h_sim;
	...
	Flamingo25(model, :settle, 0, h_sim, u, w, contact, :foot_1, :foot_2,
		copy(q0), copy(q1), qref, xdref, mpc_opts)
end

function policy(p::Flamingo25, x, traj, t)
	nc = p.model.dim.c
	iθ = Vector(3:9)
	il1 = [2,3,6]
	il2 = [4,5,7]
