...
u = [0.0*ones(model.dim.u) for t=1:H_sim]
# p = open_loop_policy(u, N_sample=N_sample)
p = flamingo_policy(model, qref=q0_sim)
...
plot!(plt[1,1], hcat(Vector.([q[1:1] for q in sim.traj.q])...)', color=:blue, linewidth=1.0)
...
visualize_robot!(vis, model, sim.traj, name=:mpc)
...
# function virtual_actuator_torque(model::Flamingo, f::AbstractVector; body=:foot_1)
# 	fz, fθ = f
# 	J = jacobian_map(model, q0_sim, body=body)
#
# 	if body == :foot_1
# 		ifoot = 6
# 		iJ = [1,2,3]
