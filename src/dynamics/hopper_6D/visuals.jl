function plot_lines!(vis::Visualizer, model::Hopper6D, q::AbstractVector;
		r_foot=0.04, offset=0.04, size=10, name::Symbol=:hopper_6D, col::Bool=true)
...
function build_robot!(vis::Visualizer, model::Hopper6D; name::Symbol=:Hopper6D, r=0.04)
	n_leg = 100
	r = convert(Float32, r)
...
function set_robot!(vis::Visualizer, model::Hopper6D, q::AbstractVector;
		name::Symbol=:Hopper6D, r=0.04)
	n_leg = 100
	r = convert(Float32, r)
	r_leg = convert(Float32, 0.5 * r)
	p_shift = [0.0; 0.0; r]
	...
	r_range = range(0, stop = q[6], length = n_leg)
    for i = 1:n_leg
		q_tmp = Array(copy(q))
		q_tmp[6] = r_range[i]
...
function contact_point(model::Hopper6D, q::AbstractVector)
	p_foot = kinematics(model, q)
	pc = [p_foot]
	return pc
end
