function build_robot!(vis::Visualizer, model::Hopper3DQuaternion; name::Symbol=:Hopper3DQuaternion, r=0.04, α=1.0)
	n_leg = 100
	r = convert(Float32, r)
...
function set_robot!(vis::Visualizer, model::Hopper3DQuaternion, q::AbstractVector;
		name::Symbol=:Hopper3DQuaternion, r=0.04)
	n_leg = 100
	r = convert(Float32, r)
	r_leg = convert(Float32, 0.5 * r)
	p_shift = [0.0; 0.0; r]
	...
	r_range = range(0, stop = q[8], length = n_leg)
    for i = 1:n_leg
		q_tmp = Array(copy(q))
		q_tmp[8] = r_range[i]
...
function contact_point(model::Hopper3DQuaternion, q::AbstractVector)
	p_foot = kinematics(model, q)
	pc = [p_foot]
	return pc
end
