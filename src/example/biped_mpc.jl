...
cost0 = CostFunction(H, model.dim,
    q = [Diagonal(1.0e-1*[0.5,0.01,0.1,.5,.5,.5,.5,.5,.5,]) for t = 1:m_opts0.H_mpc],
    u = [Diagonal(3.0e-1 * ones(nu)) for t = 1:m_opts0.H_mpc],
...
plot!(plt[1,1], hcat(Vector.(vcat([fill(ref_traj.q[i][1:nq], m_opts0.N_sample) for i=1:H]...))...)',
    color=:red, linewidth=3.0)
plot!(plt[1,1], hcat(Vector.([q[3:3] for q in mpc0.q_sim])...)', color=:blue, linewidth=1.0)
