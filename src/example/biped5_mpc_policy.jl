...
ref_traj = get_trajectory("biped5", "gait1", load_type=:split_traj, model=model)
...
cost = CostFunction(H_mpc, model.dim,
    ...
    q = [Diagonal(1e-1 * [1.0, 0.01, 0.05, 1.5, 1.5, .15, .15]) for t = 1:H_mpc],
    u = [Diagonal(3e-1 * [10; 1; 10; ones(nu-3)]) for t = 1:H_mpc],
