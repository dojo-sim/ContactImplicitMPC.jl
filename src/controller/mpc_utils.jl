...
# 	cache.q[1] .= copy(traj.q[1])
#     cache.u[1] .= copy(traj.u[1])
#     cache.w[1] .= copy(traj.w[1])
#     cache.γ[1] .= copy(traj.γ[1])
#     cache.b[1] .= copy(traj.b[1])
#     cache.z[1] .= copy(traj.z[1])
#     cache.θ[1] .= copy(traj.θ[1])
...
function rot_n_stride!(traj::ContactTraj, stride::Vector{T}) where T
    rotate!(traj)
    mpc_stride!(traj, stride)
    return nothing
end
...
# function get_stride(model, traj::ContactTraj) #TODO: dispatch over environment / model
...
function update_altitude!(alt, s::Simulation,  traj, t, N_sample;
	...
	for i = 1:s.model.nc
		...
		if γ_max > threshold
			alt[i] = ϕ_func(s.model, s.env, traj.q[idx_max+2])[i] #TODO check this this is correct
			# alt[i] = ϕ_func(model, traj.q[idx_max+2])[i]
			verbose && println(" ")
			verbose && println("point $i in contact")
			verbose && println("sim_step : $idx_max")
			verbose && println("alt      : $(ϕ_func(s.model, s.env, traj.q[idx_max+2])[i])")
			verbose && println("force    : $(traj.γ[idx_max][i])")
		end
	end
end

function live_plotting(model, ref_traj::ContactTraj,
		sim_traj, newton::Newton, q0::AbstractVector{T},
		q1::AbstractVector{T}, t::Int) where {T}
	nq = model.nq
	nu = model.nu
