...
function dummy_mpc(model::ContactDynamicsModel, core::Newton, mpc::MPC13)
    ...
    for l = 1:m_opts.M
        ...
        @show scn.(u_zoh[1], digits=2)
        sim = simulator(model, SVector{nq}(deepcopy(mpc.q_sim[end-1])), SVector{nq}(deepcopy(mpc.q_sim[end])), h_sim, N_sample,
            # p = open_loop_policy([SVector{model.dim.u}(deepcopy(core.traj.u[1]/N_sample)) for i=1:N_sample], h_sim),
        ...
        @show scn.(norm(ref_traj.q[3] - sim.traj.q[end]), digits=2)
        ########################
...
im_traj0 = ImplicitTraj(ref_traj, model, κ=κ)
n_opts0 = NewtonOptions(r_tol=3e-4, κ_init=κ, κ_tol=2κ, solver_inner_iter=5)
core0 = Newton(H, h, model, cost=cost0, opts=n_opts0)

q0_dist = deepcopy(ref_traj0.q[1])
q1_dist = deepcopy(ref_traj0.q[2])

@time newton_solve!(core0, model, im_traj0, ref_traj0,
    verbose=true, q0=q0_dist, q1=q1_dist, initial_offset=false)
@time newton_solve!(core0, model, im_traj0, ref_traj0,
    verbose=true, q0=q0_dist, q1=q1_dist, warm_start=true, initial_offset=true)

visualize!(vis, model, core0.traj.q, Δt=core.traj.h)


ref_traj0 = deepcopy(ref_traj)
@show scn.([ref_traj0.u[i][1] for i = 1:5], digits=2)
