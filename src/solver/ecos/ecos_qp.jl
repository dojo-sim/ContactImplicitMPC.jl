...
mutable struct ConeStructure15{T}
    k::Int # number of cones
    m::Int # joint cone dimensions
    mi::AbstractVector{Int} # individual cone dimensions
    cones::Cone15{T} # ordered list of cones
end

abstract type Cone15{T}
end

mutable struct PositiveOrthantCone15{T}
    m::Int # cone dimension
    e::Vector{T}
end

mutable struct SecondOrderCone15{T}
    m::Int # cone dimension
    e::Vector{T}
end
...
function combined_direction!(Δ::Direction15, r::Residual15, s::Solution15,
    ...
    dy = - (1 - η) * r.ry
    dz = - (1 - η) * r.rz
    ds = - cone_prod(λ, λ) - γ * cone_prod(inv(W)' * Δ.Δs, W * Δ.Δz) + σ * μ .* e
