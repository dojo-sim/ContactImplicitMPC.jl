...
@with_kw mutable struct InteriorPointBaseOptions{T} <: AbstractIPOptions
    r_tol::T = 1.0e-5
    κ_tol::T = 1.0e-5
    κ_init::T = 1.0
    κ_scale::T = 0.1
...
mutable struct InteriorPointBase{T} <: AbstractIPSolver
    s::Space
...
function interior_point_base(z, θ;
        ...
        opts::InteriorPointBaseOptions = InteriorPointBaseOptions()) where T
...
function interior_point_solve!(ip::InteriorPointBase{T}) where T
