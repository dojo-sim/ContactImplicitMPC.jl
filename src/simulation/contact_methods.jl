...
function velocity_stack(model::Model, env::Environment{<:World,LinearizedCone}, q1, q2, k, h)
	nc = model.nc
	ne = dim(env)
...
function velocity_stack(model::Model, env::Environment{<:World,NonlinearCone}, q1, q2, k, h)
	nc = model.nc
	ne = dim(env)
	v = J_func(model, env, q2) * (q2 - q1) / h[1]
