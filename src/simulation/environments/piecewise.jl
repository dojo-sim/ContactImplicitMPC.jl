...
function piecewise1_smoothed(x)
...
function d_piecewise1_smoothed(x)
	IfElse.ifelse(x[1] < 0.4, 0.0,
...
piecewise1_2D_lc = Environment{R2, LinearizedCone}(piecewise1_smoothed, d_piecewise1_smoothed)
...
piecewise2_2D_lc = Environment{R2, LinearizedCone}(piecewise2, d_piecewise2)
