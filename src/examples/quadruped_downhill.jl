...
s = get_simulation("quadruped", "piecewise2_2D_lc", "downhill",
	model_variable_name="quadruped_downhill",
	dynamics_name="dynamics_downhill",
	approx = true)
model = deepcopy(s.model)
env = s.env
nq = model.dim.q
nc = model.dim.c
...
obj = TrackingVelocityObjective(model, env, H_mpc,
	v = [Diagonal(1e-3 * [0.01; 0.01; 0.01; ones(4); 1; 1; 1; 1]) for t = 1:H_mpc], #safest
	q = [Diagonal(1e-2 * [5; 0.02; 0.05; 0.25 * ones(4); 0.25; 0.25; 0.25; 0.25]) for t = 1:H_mpc], #safest
	# v = [Diagonal(1e-2 * [0.01; 0.01; 0.01; 1 * ones(nq - 3)]) for t = 1:H_mpc], #best
	# q = [Diagonal(1e-2 * [5; 0.02; 0.05; 0.25 * ones(4); 0.25 * ones(4)]) for t = 1:H_mpc], #best
	# q = [Diagonal(1e-2 * [5; 0.02; 0.05; 0.25 * ones(nq-3)]) for t = 1:H_mpc],
	# q = [Diagonal(1e-2 * [5; 0.02; 0.10; 0.25 * ones(nq-3)]) for t = 1:H_mpc],
    # q = [Diagonal(1e-2 * [10; 0.02; 0.25; 0.25 * ones(nq-3)]) for t = 1:H_mpc],
	u = [Diagonal(3e-2 * [1; 1; 1; 1; 0.3; 1; 0.3; 1;]) for t = 1:H_mpc], #safest
    # u = [Diagonal(3e-2 * ones(model.dim.u)) for t = 1:H_mpc], #safest
