...
s = get_simulation("racecar", "flat_2D_lc", "flat")
model = s.model
env = s.env
nq = s.model.dim.q
nu = model.dim.u
...
q0_ref = SVector{nq,T}([0.1, 0.50, 0.2])
q1_ref = SVector{nq,T}([0.1, 0.50, 0.2])

α = 0.0077 # N=46 h=0.01

u_ref = [zeros(1) for k = 1:H]
