include(joinpath(@__DIR__, "..", "dynamics", "hopper_6D", "visuals.jl"))
T = Float64
vis = Visualizer()
open(vis)

# get hopper model
model_sim = get_model("hopper_6D", surf="sinusoidal")
model = get_model("hopper_6D")
nq = model.dim.q
nu = model.dim.u
nc = model.dim.c
nb = model.dim.b
...
# get trajectory
# ref_traj = get_trajectory("hopper_6D", "gait_in_place", load_type=:joint_traj)
ref_traj = get_trajectory("hopper_6D", "gait_forward", load_type=:joint_traj)
...
cost = CostFunction(H_mpc, model.dim,
    q = [Diagonal(1.0e-1 * [10,10,0.1,5e+1,5e+1,10])   for t = 1:H_mpc],
...
plot_surface!(vis, model_sim.env, n=200, ylims=[-0.3, 0.6])
visualize_robot!(vis, model, sim.traj, sample=20)
