...
# get trajectory
ref_traj = get_trajectory("biped5", "gait3", load_type=:split_traj, model=model)
...
obj = TrackingObjective(H_mpc, model.dim,
    ...
    q = [Diagonal(1e-2 * [0.1, 0.1, 0.5, 1.5, 1.5, .15, .15]) for t = 1:H_mpc],
    u = [Diagonal(10e-1 * [1; 1; 1; ones(nu-3)]) for t = 1:H_mpc],
...
anim = visualize_robot!(vis, model_sim, ref_traj.q)
